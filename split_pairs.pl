...
\$ perl split_pairs.pl -i samples/unsorted.fastq.gz -B 1G

\$ perl split_pairs.pl -i samples/sorted.fastq -n -l sorted.12.fq -R "(^.*?)#\\w+?(/[12])" -S "\\1/\\2" 

\$ perl split_pairs.pl -i samples/sorted.fastq -n -1 reads1.fq -2 reads2.fq -s singles.fq -z 8

\$ perl split_pairs.pl -i samples/sorted1.9.fq.gz -p " "
